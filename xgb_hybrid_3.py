...
# At a check point, if the memory impact of the hybrid policy/the impact of the deterministic < 2
# for the WHOLE HISTORY, then run ML.
# Else, run deterministic.


def xgb_hybrid_picking3(model, window_size, sequence, k,
    ...
    while pointer < len(sequence):
        ...
        if int(pointer / check_length) > seg:
            ...
            if run_oracle and total_impact / det_req2mi[pointer - 1] > 2:
                deterministic_current_box = det_req2box[pointer]
                deterministic_box_countings = det_req2counter[pointer]
            run_oracle = (total_impact / det_req2mi[pointer - 1]) <= 2
...
os.makedirs(time_str + '-result-plots-hybrid3')
...
for trace_name in names:
    ...
    test_seq = [i for i in range(6000000)]
    ##########################
    '''
    ...
    xgb_hybrid_impact, sr, xgb_m_boxes = xgb_hybrid_picking3(model, window_size, test_seq, k, number_of_box_kinds,
    ...
    for index, value in enumerate(v):
        plt.text(index - 0.07, value + 0.1, str(round(value, 2)),
                 fontdict={'weight': 'black'})
    # plt.show()
    plt.savefig(r'' + time_str + '-result-plots-hybrid3/1-v-all-s' + str(miss_cost) + 'd' + str(params['max_depth']) + 'r' +
